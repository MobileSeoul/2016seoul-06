//This is source code of favorite. Copyright?Î±?. Tarks. All Rights Reserved.
...
import com.cym5240.kis.CropManager;
import com.cym5240.kis.MainActivity2;
...
public class join extends SherlockActivity implements OnCheckedChangeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Intent intent = getIntent();// ??î§?????ìÉ Ë´õÏèÜÎ∏???Ç§???
	...
	public boolean onContextItemSelected(android.view.MenuItem item) {
		...
		case 1:
			...
			// ÂØÉÍ≥å?Çµ?ëú? ?î±?ãÍΩ©?Î∏???íó Activity ??Éá?ï∞?
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == IMAGE_EDIT) {
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				// Log.i("datasetdata", data.getData().toString() + "ssdsd");
				profile.setImageBitmap(profile_bitmap); // ?Í∂óÔßû? ?ÍΩ??Íπ??Î∏? ?Í∂óÔßûÍº?RIÊø?? ?Îø∞ÂØÉÍ≥†Î∏ØÊπ??
				// Profile changed
				profile_changed = true;
	...
	// Ë´õÍπäÍ∂éÁëú? ??ãÉ????ì£?Î∏???ìΩ Ë´õÏÑè?ì≥.
	@Override
	...
	public void joinAct() {
		...
		try {
			...
			//??î†Ë™?? 	Global.dumpArray(array);

				// Setting Editor
				SharedPreferences edit = getSharedPreferences("setting",
						MODE_PRIVATE);
				SharedPreferences.Editor editor = edit.edit();
				editor.putString("frist_use_app", "false"); // ?Ç§?ç†?èô?òô,
