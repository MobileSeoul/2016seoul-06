...
import com.cym5240.kis.page.document_read;
	...
	protected void onMessage(Context arg0, Intent arg1) {
		...
		while (iterKey.hasNext()) {
			...
			if (key.matches("data")) {
				...
				if (des.matches("new_document"))
					des = getString(R.string.notice_new_document);
				if (des.matches("new_comment"))
					des = getString(R.string.notice_new_comment);
				if (des.matches("added_to_favorite")) {
					des = getString(R.string.notice_added_to_favorite);
					content = getString(R.string.notice_added_to_favorite);
				}
		...
		String user_explain = String.format(
				res.getString(R.string.user_explain), title);
		...
		Notification builder = new NotificationCompat.Builder(
				...
				.setContentText(des)
				.setLargeIcon(
						Global.filetobitmap(getCacheDir().toString()
								+ "/member/thumbnail/" + send_user_srl + ".jpg"))
				...
				.setStyle(
						new NotificationCompat.BigTextStyle().bigText(Global
								.getValue(content))).build();
		...
		// Ringtone ringtone =
		// RingtoneManager.getRingtone(getApplicationContext(), ringtoneUri);
		// ringtone.play();
		//
